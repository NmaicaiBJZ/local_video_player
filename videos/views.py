...
from .utils import generate_thumbnail_pyav, generate_classified_thumbnail
...
def index(request):
    ...
    return render(request, 'videos/index.html', {"video_folders": video_folders, "video_files": video_files, "current_path": None})

def video_folder_view(request, file_path):
    video_folders, video_files = file_processing(file_path)
    # print(file_path)
    return render(request, 'videos/index.html', {"video_folders": video_folders, "video_files": video_files, "current_path": os.path.dirname(file_path)})
...
def file_processing(file_location):
    ...
    for f in os.listdir(folder_path):
        ...
        if f.endswith('.mp4'):
            ...
            # 分类缩略图的图片文件夹，需要将这个文件夹下的所有缩略图进行汇总
            classified_thumbnail_folder_path = os.path.join(thumbnail_folder_path, f)
            # 生成出来的缩略图集合图片地址
            folder_thumbnails_path = os.path.join(classified_thumbnail_folder_path, f'{f}_thumbnails.jpg')
            if not os.path.exists(folder_thumbnails_path):
                generate_classified_thumbnail(classified_thumbnail_folder_path, folder_thumbnails_path, 300, 200)
            insert_text = {
                "folder_name": f,
                "folder_path": url_path,
                "thumbnail_path": os.path.join(settings.MEDIA_URL, 'thumbnails', f, f'{url_path}_thumbnails.jpg')
            }

            video_folders.append(insert_text)
